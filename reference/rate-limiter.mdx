...
description: "How checkRateLimit works in ERP B2B Premium — in-process sliding window rate limiting for Server Actions with configurable identifier, limit, and window."
---

`checkRateLimit` (`src/lib/utils/rate-limiter.ts`) provides lightweight in-process rate limiting for Server Actions. It uses an in-memory `Map` with a sliding window to track request counts per identifier. It is used in the wizard to prevent form submission abuse.
...
3. **Within window** — if the entry is active and below the limit, `count` is incremented and `{ allowed: true, remaining: maxRequests - count }` is returned.
