...
A ±15% range is applied to the estimated total to produce the budgetary min/max band:

```
rangeMin = estimatedTotal × 0.85
rangeMax = estimatedTotal × 1.15
```
...
- `volumeCubicMeters` is clamped between **0.001** and **10,000,000 m³**. Values outside this range are silently adjusted and a warning is logged.
