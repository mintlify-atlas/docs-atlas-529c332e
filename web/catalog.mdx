...
## Public access and RLS

The catalog is designed for anonymous read access. Unauthenticated users can browse the full product hierarchy without authentication.

```
Anonymous user → getIndustrialCatalog() → supabaseAdmin (read)
                                            ↑
                           RLS: SELECT allowed for all, no tenant filter needed
                           (catalog is scoped by tenantId in the query itself)
```

Any attempt to `INSERT`, `UPDATE`, or `DELETE` from an anonymous context will be rejected by the RLS policies configured on `products`, `product_categories`, and related tables.
