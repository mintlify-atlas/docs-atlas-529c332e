  ...
  <Tab title="Vercel">
    ...
    <Steps>
      ...
      <Step title="Add environment variables">
        ...
        vercel env add NEXT_PUBLIC_DEFAULT_TENANT_CODE
        ```
      </Step>
...
The GitHub Actions workflow at `.github/workflows/ci.yml` runs automatically on every push and pull request to `main` / `master`. It executes four quality gates in sequence:
...
| 4 | `npm audit --audit-level=high` | Security audit — flags `high`+ vulnerabilities |
...
The script verifies that all required environment variables are set, then runs `tsc --noEmit`, `lint`, `vitest run`, and `npm run build` in order. It exits non-zero on the first failure.
