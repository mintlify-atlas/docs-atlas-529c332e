...
`stop()` returns the elapsed time in milliseconds so you can include it in your own response metrics.
...
|------|-------------|-------------|
| `VALIDATION_ERROR` | 400 | Invalid or malformed input |
| `AUTH_ERROR` | 401 | Unauthenticated request / expired session |
| `FORBIDDEN` | 403 | Authenticated but not authorized |
| `NOT_FOUND` | 404 | Resource does not exist |
| `RATE_LIMITED` | 429 | Too many requests |
| `TENANT_MISMATCH` | 403 | Cross-tenant data access attempt |
| `INTERNAL_ERROR` | 500 | Unexpected server error (default) |

### Server Action response convention

All Server Actions in the project return a discriminated union:
...
Use `toActionResponse()` from `app-error.ts` to wrap any promise and convert thrown `AppError` instances into the error shape automatically.
